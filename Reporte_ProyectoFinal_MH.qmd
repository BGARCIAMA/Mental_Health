...
3.  Entrenar un modelo de machine learning para predecir la depresión en adultos.
...
Cabe destacar que el conjunto de test, no incluye la variable objetivo para la predicción (`Depression`).
...
-   El análisis de la variable objetivo `depression` revela un conjunto de datos ligeramente desbalanceado, donde aproximadamente el 82% de los registros corresponden a individuos sin depresión y solo el 18% a casos positivos. Esta desigualdad es visualmente evidente en la gráfica de distribución.
...
for i, feature in enumerate(num_feats):
    ...
    sns.histplot(
        ...
        alpha=0.4,
        color="#66b7ee",
        line_kws={"lw": 1, "linestyle": "dashed"},
    ).lines[0].set_color("darkred")
    plt.tick_params(axis="both", labelsize=9)
    plt.ylabel("Porcentaje", fontsize=9)
    plt.xlabel(feature, fontsize=9)
fig.tight_layout()
plt.show()
```

-   Con estos histogramas podemos analizar las distribuciones de los datos y detectar patrones o anomalías en las características numéricas, e identificar las que en realidad son categóricas.

<!-- -->

-   Para la variable de `age` la distribución es aproximadamente uniforme, aunque hay un ligero aumento en la frecuencia en el rango de los 40 años.

-   Para las variables como `academic_pressure`, `work_pressure`, `study_satisfaction`, `job_satisfaction` y `financial_stress` las distribuciones son multimodales, lo que nos indica que están influenciadas por factores externos que generan diferentes "grupos" y podemos considerarlas como categóricas ya que probablemente en la encuesta manejaron las variables en escala del 1 al 5.
...
-   Los hábitos alimenticios de las personas son diversos, distribuidos entre categorías moderadas, saludables y no saludables de manera bastante equilibrada.

-   En cuanto a los patrones de sueño, la mayoría de los individuos reportan dormir entre 5 y 8 horas por noche, lo que se encuentra dentro de un rango considerado saludable. Esto refleja una tendencia general hacia comportamientos de vida relativamente equilibrados.

-   Un análisis sobre los aspectos de salud mental revela cifras preocupantes. Una proporción significativa de la población reporta haber tenido pensamientos suicidas, con una distribución casi equilibrada entre respuestas afirmativas y negativas.

-   Además, aproximadamente la mitad de las personas indican antecedentes familiares de enfermedades mentales. Estas estadísticas resaltan la importancia de investigar más a fondo los factores asociados con la salud mental.
...
CatBoost es un algoritmo de aprendizaje automático basado en árboles de decisión que utiliza técnicas avanzadas de boosting. Es especialmente útil para trabajar con datos categóricos, ya que implementa un manejo eficiente de estas variables mediante codificación *Ordered Target Encoding*, lo que reduce la necesidad de preprocesamiento adicional. Además, CatBoost es conocido por su capacidad de minimizar el riesgo de sobreajuste y su alto desempeño en tareas de clasificación y regresión.

Elegimos probar CatBoost para este ejercicio debido a la naturaleza del dataset, que contiene una combinación de variables numéricas y categóricas. Para entrenar el modelo, el dataset se preprocesó llenando valores faltantes con "0" y ajustando los tipos de datos según la naturaleza de las variables (numéricas, categóricas y objetivo).
...
Para posteriormente entrenar el modelo en partes, utilizando validación cruzada estratificada, con los parámetros obtenidos por *Optuna* para luego predecir la probabilidad de depresión en un conjunto de datos de prueba independiente. Básicamente divide los datos en grupos, entrena el modelo en un grupo y valida su rendimiento en otro, repitiendo esto para 20 grupos.
...
Dando como principales indicadores para medir el desempeño del modelo lo siguiente:
...
![ROC CatBoosts](img/cat_boost.png)

Para evaluar el desempeño del modelo de clasificación, se construyó una curva ROC (Receiver Operating Characteristic). Esta curva ilustra la relación entre la tasa de verdaderos positivos (sensibilidad) y la tasa de falsos positivos (1-especificidad) para diferentes umbrales 1  de clasificación. El área bajo la curva ROC (AUC) es una medida resumida del rendimiento del modelo, donde un valor cercano a 1 indica una excelente capacidad discriminatoria. En este caso, el modelo obtuvo un AUC de $0.9758$, lo cual sugiere un alto poder predictivo. 

Sin embargo, en el contexto de la detección de la depresión, minimizar la tasa de falsos negativos es de suma importancia. Un falso negativo implica que el modelo clasifica erróneamente a un individuo con depresión como no deprimido, lo que podría retrasar el acceso a tratamientos adecuados. Por esta razón, buscamos mover el threshold para la clasificación hacia la derecha incrementando el _False Positive Rate_.

Para identificar un punto de corte adecuado, se realizó un análisis visual de la curva ROC y calculando el punto óptimo en la curva se obtuvo aproximadamente $0.25$ como Threshold. Este punto de corte permite obtener una alta sensibilidad, es decir, una alta probabilidad de detectar correctamente a los individuos con depresión, al tiempo que se mantiene una tasa de falsos positivos aceptable.

![Matrix CatBoosts](img/matrix_catboots.png)

Es importante destacar que la elección del punto de corte implica un equilibrio entre sensibilidad y especificidad. Un punto de corte más conservador (menor tasa de falsos positivos) podría disminuir la cantidad de falsos negativos, pero también reduciría la sensibilidad del modelo. Por tanto, la decisión final debe considerar las implicaciones clínicas y los costos asociados a los falsos positivos y falsos negativos.
